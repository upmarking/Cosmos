    ...
    googleBtn.addEventListener('click', async () => {
      try {
        ...
        if (cred.user && !cred.user.emailVerified) {
          ...
          googleBtn.innerHTML = defaultBtnHtml;
          return;
        }
        ...
        if (err.code === 'auth/popup-closed-by-user' || err.code === 'auth/cancelled-popup-request') {
          ...
          showToast('An account already exists with this email. Please sign in with your password.', 'error');
  ...
  if (signupForm) {
    signupForm.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        await signOut(auth);
        showToast('Account created! A verification link has been sent. Please verify it to log in.', 'success');
        currentView = 'login';
        renderAuth(outlet);
      } catch (err) {
        showToast(getAuthError(err.code), 'error');
