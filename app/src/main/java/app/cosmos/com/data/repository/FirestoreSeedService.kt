...
object FirestoreSeedService {
    ...
    suspend fun seedUsers(firestore: FirebaseFirestore) {
        val users = listOf(
            mapOf(
                ...
                "followersCount" to 14,
                "followingCount" to 14,
                "eventsAttended" to 5,
            ...
            mapOf(
                ...
                "followersCount" to 42,
                "followingCount" to 42,
            ...
            mapOf(
                ...
                "followersCount" to 19,
                "followingCount" to 19,
                "eventsAttended" to 8,
    ...
    suspend fun seedSocialPosts(firestore: FirebaseFirestore) {
        ...
        // Check if we need to seed or migrate existing seeded posts
        val postsSnapshot = firestore.collection("social_posts").get().await()
        val needsReseed = postsSnapshot.isEmpty || postsSnapshot.documents.any { !it.contains("isLinkedInConnected") }
        if (!needsReseed) return

        // Clear existing posts & replies to migrate/re-seed clean data
        if (!postsSnapshot.isEmpty) {
            for (doc in postsSnapshot.documents) {
                try {
                    val replies = doc.reference.collection("replies").get().await()
